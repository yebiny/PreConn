...
while continueRoutine:
    ...
    if welcome_resp.status == STARTED and not waitOnFlip:
        theseKeys = welcome_resp.getKeys(keyList=set_resp_list, waitRelease=False)
...
while continueRoutine:
    ...
    if wait_resp.status == STARTED and not waitOnFlip:
        theseKeys = wait_resp.getKeys(keyList=set_resp_list, waitRelease=False)
...
for i, thisBlock in zip(range(nBlocks), Blocks):
    ...
    for j, thisImg in zip(range(nImgs), Imgs):
        ...
        if idx%12==0: base_time  = rest_time + btw_blocks_time 
        else: base_time  = rest_time
        ...
        print(i, j, idx, this_img, target_list[idx], stim_category)

        # ------Prepare to start Routine "Trial"-------
        continueRoutine = True
        routineTimer.add(img_time)
        # update component parameters for each repeat
        Resp.keys = []
        Resp.rt = []
        _Resp_allKeys = []
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if Resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Resp.tStartRefresh + img_time-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if Resp.status == STARTED and not waitOnFlip:
                theseKeys = Resp.getKeys(keyList=sub_resp_list, waitRelease=False)
            ...
            if Dot.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Dot.tStartRefresh + img_time-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        continueRoutine = True
        routineTimer.add(base_time+sig_resp_term)
        # update component parameters for each repeat
        rest_resp.keys = []
        rest_resp.rt = []
        _rest_resp_allKeys = []
        # keep track of which components have finished
        RestComponents = [rest_dot, rest_resp]
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if rest_dot.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rest_dot.tStartRefresh + base_time+sig_resp_term-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # *rest_resp* updates
            waitOnFlip = False
            if rest_resp.status == NOT_STARTED and tThisFlip >= base_time-sig_resp_term-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rest_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rest_resp.tStartRefresh + 0.2-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rest_resp.status == STARTED and not waitOnFlip:
                theseKeys = rest_resp.getKeys(keyList=sig_resp_list, waitRelease=False)
        ...
        for thisComponent in RestComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        Imgs.addData('rest_dot.started', rest_dot.tStartRefresh)
        Imgs.addData('rest_dot.stopped', rest_dot.tStopRefresh)
        # check responses
        if rest_resp.keys in ['', [], None]:  # No response was made
            rest_resp.keys = None
        Imgs.addData('rest_resp.keys',rest_resp.keys)
        if rest_resp.keys != None:  # we had a response
            Imgs.addData('rest_resp.rt', rest_resp.rt)
        Imgs.addData('rest_resp.started', rest_resp.tStartRefresh)
        Imgs.addData('rest_resp.stopped', rest_resp.tStopRefresh)
        thisExp.nextEntry()
